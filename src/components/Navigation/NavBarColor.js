...
function updateNavBarColor (favicons, tabId) {
  // private AppState always use a special color, we don't need to get the icon
  if (AppState.get(tabId).private === true) {
    return
  }
...
export function updateColorPalette () {
  var tab = AppState.get(AppState.getSelected())

  if (tab.private) {
    // private AppState have their own color scheme
